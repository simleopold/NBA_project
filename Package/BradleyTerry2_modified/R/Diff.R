...
Diff <- function(player1, player2, formula = NULL, id = "..", data = NULL,
    ...
    if (!empty) {
        ...
        if (!is.null(formula)){
        # Extract the terms from the formula object
        formula_terms <- attr(terms(formula), "term.labels")
        
        # Now remove any occurrences of '[..]' from the variable names
        metrics_home <- gsub("\\[.*\\]", "", formula_terms)
        
        # Create away team metrics (with 'a' suffix)
        metrics_away <- paste0(metrics_home, "a")
        
        metrics_home <- metrics_home[1:length(metrics_home) -1]
        metrics_away <- metrics_away[1:length(metrics_away) -1]
        # Now, use the same logic for creating mf1 and mf2
        # Create new mf1 (Home Team stats)
        mf1 <- data[, c("home_team", metrics_home)]
        colnames(mf1)[1] <- "team"  # Rename 'home_team' to 'team'
        
        # Create new mf2 (Away Team stats)
        mf2 <- data[, c("away_team", metrics_away)]
        colnames(mf2) <- c("team", metrics_home)  # Rename 'away_team' to 'team' and remove 'a' from metrics
        
        X1 <- model.matrix(~ . - 1, data = mf1[, metrics_home, drop = FALSE])
        X2 <- model.matrix(~ . - 1, data = mf2[, metrics_home, drop = FALSE])
        
        # Select the metrics for home and away teams
        #metrics_home <- c("AST", "TOV", "X3P")
        #metrics_away <- paste0(metrics_home, "a")  
        ...
        # new matrix for my data
        #X1 <- model.matrix(~ AST + TOV + X3P, data = mf1)
        #X2 <- model.matrix(~ AST + TOV + X3P, data = mf2)
    ...
    list(X = X, random = random, offset = offset,
         ...
         saturated = saturated)
    #print(list(X = X, random = random, offset = offset,
     #          term.labels = term.labels, refcat = refcat, contrasts = contrasts,
      #         saturated = saturated))
    
    #$X --> difference in. predictors (winner - loser or home - away)
    #all good with the player variable 
}
